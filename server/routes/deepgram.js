...
export function setupWebSocket(server) {
  ...
  wss.on('connection', async (ws, req) => {
    ...
    let isDeepgramReady = false; // ✅ Track if Deepgram is ready for audio
      ...
      try {
        ...
        if (data.type === 'start') {
          ...
          try {
            ...
            deepgramConnection.on('open', () => {
              ...
              isConnecting = false;
              isDeepgramReady = true; // ✅ Mark as ready
              ws.send(JSON.stringify({ 
                type: 'status', 
                message: 'connected',
            ...
            deepgramConnection.on('Transcript', (transcriptData) => {
        ...
        if (data.type === 'audio' && deepgramConnection) {
          ...
          if (readyState === 1 && isDeepgramReady) {
            ...
            console.warn('   Ready state:', readyState, '(0=CONNECTING, 1=OPEN, 2=CLOSING, 3=CLOSED)');
